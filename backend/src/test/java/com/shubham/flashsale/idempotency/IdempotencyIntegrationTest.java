...
class IdempotencyIntegrationTest {
    ...
    void shouldReplayCompletedResponseForSameUserAndSameIdempotencyKey() throws Exception {
        ...
        performPurchase(USER_A_UUID, "test-key")
        ...
        verify(purchaseService, times(1))
                .purchase(anyString(), anyString(), eq("test-key"), any());
    }
    ...
    void shouldReleaseProcessingKeyAfterServerErrorSoRequestCanBeRetried() throws Exception {
        ...
        performPurchase(USER_A_UUID, "failure-key")
                .andExpect(status().isInternalServerError());

        performPurchase(USER_A_UUID, "failure-key")
                .andExpect(status().isOk());

        verify(purchaseService, times(2))
                .purchase(anyString(), anyString(), eq("failure-key"), any());
    }
    ...
    void shouldIsolateSameIdempotencyKeyAcrossDifferentUsers() throws Exception {
        ...
        performPurchase(USER_B_UUID, "test-key")
                .andExpect(status().isOk());
    ...
    private org.springframework.test.web.servlet.ResultActions performPurchase(
        ...
        return mockMvc.perform(post(PURCHASE_URL)
                .with(jwt().jwt(jwt -> jwt
                        .subject(userUuid)
                        .claim("role", "USER")))
