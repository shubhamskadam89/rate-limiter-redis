  ...
  public RateLimitIdentity resolve(HttpServletRequest request) {
    ...
    if (auth != null && auth.isAuthenticated() && !(auth instanceof AnonymousAuthenticationToken)) {
      ...
      if (auth.getPrincipal() instanceof Jwt jwt) {
        RateLimitIdentity identity = new RateLimitIdentity(IdentityType.USER, jwt.getSubject());
        log.debug("Resolved rate limit identity: {}", identity);
        return identity;
      }
    }
    RateLimitIdentity identity = new RateLimitIdentity(IdentityType.IP, request.getRemoteAddr());
    log.debug("Resolved rate limit identity: {}", identity);
    return identity;
  }
}
