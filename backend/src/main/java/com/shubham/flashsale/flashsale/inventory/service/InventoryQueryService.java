...
public class InventoryQueryService {
  ...
  public InventoryResponse getInventory(String saleItemUuid) {
    ...
    if (saleEvent.getStatus() == Status.ENDED
        || saleEvent.getEndTime().isBefore(LocalDateTime.now())) {
    ...
    String redisValue =
        redisTemplate.opsForValue().get(RedisKeyBuilder.inventory(saleItem.getUuid()));

    if (redisValue != null) {
      long remainingInventory = Long.parseLong(redisValue);
      log.debug(
          "Inventory fetched from Redis for saleItemUuid={}, remaining={}",
          saleItemUuid,
          remainingInventory);
      return InventoryResponse.builder()
          .saleItemUuid(UUID.fromString(saleItem.getUuid()))
