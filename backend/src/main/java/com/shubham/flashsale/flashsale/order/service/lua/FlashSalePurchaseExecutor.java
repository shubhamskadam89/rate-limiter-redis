...
public class FlashSalePurchaseExecutor {
  ...
  public PurchaseResult execute(
      String saleItemUuid, String userUuid, int quantity, int maxPerUser) {
    log.debug(
        "Executing Lua purchase script for saleItemUuid={}, userUuid={}, quantity={}",
        saleItemUuid,
        userUuid,
        quantity);
    ...
    if (first == -1) {
      log.warn(
          "Purchase failed: SOLD_OUT for saleItemUuid={}, userUuid={}", saleItemUuid, userUuid);
      return new PurchaseResult(PurchaseStatus.SOLD_OUT, 0, (int) second);
    }

    if (first == -2) {
      log.warn(
          "Purchase failed: LIMIT_EXCEEDED for saleItemUuid={}, userUuid={}",
          saleItemUuid,
          userUuid);
    ...
    log.debug(
        "Lua purchase script execution SUCCESS for saleItemUuid={}, userUuid={}, remaining={}",
        saleItemUuid,
        userUuid,
        first);
