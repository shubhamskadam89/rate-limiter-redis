...
public class SaleServiceImpl implements SaleService {
  ...
  public SaleResponse createSale(CreateSaleRequest request) {
    ...
    log.debug(
        "Validating sale request name={} startTime={} endTime={}",
        request.getName(),
    ...
    log.info(
        "Creating sale campaign name={} by user={}", request.getName(), currentUser.getEmail());
  ...
  public SaleItemResponse addItemToSale(String saleUuid, AddSaleItemRequest request) {
    ...
    if (saleEvent.getStatus() != Status.DRAFT) {
      log.warn(
          "Attempt to modify non-draft sale saleId={} status={}", saleUuid, saleEvent.getStatus());
      throw new SaleAlreadyActiveException(saleUuid);
    }

    Product product =
        productRepository
            .findByUuid(request.getProductUuid())
            .orElseThrow(
                () -> {
    ...
    log.debug(
        "Validating sale item request productId={} salePrice={} inventory={}",
        product.getUuid(),
    ...
    log.info(
        "Product={} added to sale={}  uuid={}", product.getUuid(), saleUuid, savedItem.getUuid());
  ...
  public SaleResponse activateSale(String saleUuid) {
    ...
    if (saleEvent.getStatus() == Status.ACTIVE) {
      log.info(
          "Sale is already active; returning current state without reinitializing inventory saleUuid={}",
          saleUuid);
      return SaleResponse.builder()
        ...
        saleItem -> {
          if (saleItem.getInventory() == null || saleItem.getInventory() <= 0) {
            throw new InvalidSaleException(
                "Sale item inventory must be positive before activation");
          }
          ...
          Boolean initialized =
              redisTemplate
                  .opsForValue()
                  .setIfAbsent(inventoryKey, String.valueOf(saleItem.getInventory()));
  ...
  @Override
  @Cacheable(cacheNames = CacheNames.SALE, key = "#saleUuid")
  ...
  @Override
  @Transactional
  @Cacheable(cacheNames = CacheNames.SALE_DETAIL, key = "#saleUuid")
  ...
  @Override
  @Transactional
  @Cacheable(cacheNames = CacheNames.ADMIN_SALES)
  public List<SaleDetailResponse> getAdminSales() {
    User currentUser = commonAuthService.getCurrentUser();

    if (currentUser.getRole().name().equals("ADMIN")) {
      return saleEventRepository.findAllByOrderByCreatedAtDesc().stream()
          .map(this::toSaleDetailResponse)
          .toList();
    }

    return saleEventRepository.findByCreatedByOrderByCreatedAtDesc(currentUser).stream()
        .map(this::toSaleDetailResponse)
        .toList();
  }
  ...
  @Override
  @Transactional
  @Cacheable(cacheNames = CacheNames.AVAILABLE_SALES)
  public List<SaleDetailResponse> getAvailableSales() {
    return saleEventRepository.findByStatus(Status.ACTIVE).stream()
        .map(this::toSaleDetailResponse)
        .toList();
  }
  ...
  @Override
  @Transactional
  @Cacheable(cacheNames = CacheNames.SALE_ITEMS, key = "#saleUuid")
  public List<SaleItemResponse> getSaleItems(String saleUuid) {
    ...
    return saleItemRepository.findBySaleEvent(saleEvent).stream()
  ...
  private SaleDetailResponse toSaleDetailResponse(SaleEvent saleEvent) {
    return SaleDetailResponse.builder()
        ...
        .endTime(saleEvent.getEndTime())
        .status(saleEvent.getStatus())
        .items(saleEvent.getSaleItems().stream().map(this::toSaleItemResponse).toList())
        .build();
  }
