...
public class CommonFlashSaleService {
  ...
  public SaleEvent validateSaleItemBelongsToSale(SaleItem saleItem, String saleUuid) {
  ...
  public Order persistOrder(User user, SaleItem saleItem, Integer quantity, String idempotencyKey) {
  ...
  public PurchaseResponse buildPurchaseResponse(
      Order savedOrder, SaleItem saleItem, PurchaseResult result) {
  ...
  public void validateSaleRequest(CreateSaleRequest request) {
    if (request.getEndTime().isBefore(request.getStartTime())) {
      log.debug(
          "Validation failed: end time before start time start={} end={}",
  ...
  public void validateSaleItemRequest(AddSaleItemRequest request, Product product) {
    ...
    if (request.getSalePrice().compareTo(product.getBasePrice()) > 0) {
      log.debug(
          "Validation failed: sale price={} exceeds base price={}",
  ...
  public PurchaseResponse buildQueuedPurchaseResponse(
      OrderQueueMessage message, SaleItem saleItem, PurchaseResult result) {
    return PurchaseResponse.builder()
